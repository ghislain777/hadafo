...
const boutonNouveau = "Nouvel enregistrement"
let page = {
    titreListe: "Emissions en direct",
...
const tab = Chaine.rawAttributes
let table = {
    nom: latable,
...
fs.mkdirSync(repertoireColones, {
    recursive: true
})
...
if (table.champImageExiste === false) {
    ...
    fs.mkdirSync(repertoireFormulaire, {
        recursive: true
    })
    ...
    fs.mkdirSync(repertoireControllerPourImage, {
        recursive: true
    })
    ...
    //view pour image
    fs.mkdirSync(repertoireViewPourImage, {
        recursive: true
    })
    fs.writeFile(fichierViewPourImage, getContenuFichierViewPourImage(table, page), function (err) {
        if (err) throw err;
    })
    //Edition pour image
    fs.mkdirSync(repertoireEditionPourImage, {
        recursive: true
    })
    ...
    fs.mkdirSync(repertoireFormulairePourImage, {
        recursive: true
    })
    fs.writeFile(fichierFormulairePourImage, getContenuFichierFormulairePourImage(table, page), function (err) {
        if (err) throw err;
    })

    // formulaire image
    fs.mkdirSync(repertoireFormulaireImage, {
        recursive: true
    })
